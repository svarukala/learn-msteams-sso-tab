...
express.get("/exchangeSsoTokenForOboToken", async (req, res) => {
    ...
    const ssoToken = req.query.ssoToken as string; //idToken
    const tokenFor = req.query.tokenFor as string;
    ...
    const oAuthOBOParams = {
      ...
      scope: tokenFor==="spo" ?  process.env.SPO_TAB_APP_SCOPES : process.env.TAB_APP_SCOPES
    };
